...
// @access Private
router.delete(
  "/api/shorters/delete/:id",
  passport.authenticate("jwt", { session: false }),
  (req, res) => {
    Shorter.findById(req.params.id)
      .then(shorter => {
        shorter.remove().then(() => res.json({ success: true }));
      })
      .catch(err => res.status(404).json({ postnotfound: "No post found" }));
  }
);

// @route PUT api/shorters/update
// @desc UPDATE shorters url by current user
// @access Private
router.put(
  "/api/shorters/update/:id",
  passport.authenticate("jwt", { session: false }),
  (req, res) => {
    Shorter.findOne({ shortUrl: req.body.shortUrl }).then(resUrl => {
      if (
        resUrl == null ||
        resUrl.originalUrl.toString() == req.body.originalUrl.toString()
      ) {
        Shorter.findByIdAndUpdate(
          req.params.id,
          req.body,
          { new: true },
          function(err, shorter) {
            if (err) return handleError(err);
            io.getIO().emit("shorterUpdate", shorter);
            return res.status(200).send(shorter);
          }
        );
      } else if (
        resUrl.originalUrl.toString() !== req.body.originalUrl.toString()
      ) {
        res.status(404).json({ Message: "Url already exist" });
      }
    });
  }
);
...
router.post(
  ...
  (req, res) => {
    ...
    if (!validUrl.isUri(originalUrl)) {
      ...
      Shorter.findOne({ urlCode: urlCode }).then(urlcode => {
        if (urlcode) {
          return res.status(400).json({ messange: "please generate again" }); //response if already
        }
        axios.get(req.body.originalUrl).then(response => {
